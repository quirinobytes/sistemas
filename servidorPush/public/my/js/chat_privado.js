...
(function() { // <== The start of the scoping function
	...
	function loadChatWith(from, to, aposNItens) {
		...
		$.ajax({
			...
			if (data){
				...
				console.log("ja tenho lido para o to="+to)
				console.log(contadorAposNItensPrivateMensagensObj.to)
				data.forEach(item => { 
					// console.log("ITEM:");
					// console.log(item)
					de = item.from
	...
	$(function(){
		...
		$( document ).ready(function() {
			...
			// Aqui estou pegando a lista dos usuários 
			loadConsumers()
		...
		socket.on('contactTo', (data) => {
			...
			if (data.toContact == friendUsername && data.from == loggedUser){
				if (data.from == loggedUser){
					// contadorAposNItensPrivateMensagensObj.from += 1
			...
			if (data.from == divContato.innerText && data.toContact == loggedUser ){
				from = data.from
				console.log("contadorAposNItensPrivateMensagensObj."+from+"="+window.contadorAposNItensPrivateMensagensObj.from)
				
				console.log("aumentando um para o ="+from)
				console.log(window.contadorAposNItensPrivateMensagensObj.from)
